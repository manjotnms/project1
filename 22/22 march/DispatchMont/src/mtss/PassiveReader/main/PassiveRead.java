package mtss.PassiveReader.main;
...
import mtss.PassiveReader.ReaderApi.*;
import mtss.PassiveReader.ReaderApi.sdk.*;
...
public class PassiveRead extends Thread {
	...
	static Logger log = LogsMtss.getLogger(PassiveRead.class.getName());
	...
	public void run() {
		...
		boolean st = false;
		do{
			st = this.callConnectReader();
			if(st) {
				log.debug("Success !!");
				break;
			} else {
				log.debug("Failed !! Another attempt.");
			}
		} while(true);
	}

	public boolean callConnectReader(){
		if( this.connectReader() ) {
			log.debug("Reading starting");
			this.connected = true;
			this.readContinously();
			return true;
		} else {
			this.connected = false;
			log.debug("Oops, not getting the connection.");
			return false;
		}
	}
	public static void main(String[] args) {
		...
		//log.debug("Got one EPC number : "+epcNum);
		//log.debug("Stop Reading");
		
		//new PassiveRead().disconnectReader();
		//log.debug("Disconnect Reader Done.");
	}
    ...
    static public String readerUsed_Ip = "";
    ...
    public void readContinously() {
    	log.debug(" readContinously called...");
    	String strTemp;
        int m_LAddress = 0;
        int m_LLen = 0;
        int m_mem = 0;//bank
        ...
        if ((m_LAddress<0) || (m_LAddress>2047))
        {
            ...
            log.debug("Please input Location of Tag Address between 0 and 2047!");
            epcTagGetter = false;
            return;
        }
        ...
        if ((m_LLen<0) || (m_LLen>2048))
        {
            ...
            //jToggleButton1.setSelected(false);
            log.debug("Please input Length of Tag Data between 0 and 2048!");
            epcTagGetter = false;
            return;
		}
		...
		case 0:
			...
			log.debug("Please choose one antenna at least!");
			return;
...
// 		case 8:
		default:
			...
			if (res!=ReaderAPI._OK)
			{
				...
				log.debug("SetAntenna Fail!Please try again!");
				return;
			}
			break;
		}
...
*/
	...
	public boolean connectReader() {
		log.debug("connect reader called...");
        ...
        java.util.ResourceBundle bundle = null, bundleHost = null;
        // Read these parameters from Resource Bundle.
        try {
        	fis = new FileInputStream("C:/Mtss/Properties/PassiveReaderConfig.properties");
        	//fis = new FileInputStream("/home/mtss/tools/PassiveReaderConfig.properties");
        	
        	fis1 = new FileInputStream("C:/Mtss/Properties/Host.properties");
        	//fis1 = new FileInputStream("/home/mtss/tools/Host.properties");
        	
        	// Read values from config file to load the machine.
    	    bundle = new PropertyResourceBundle(fis);
    	    bundleHost = new PropertyResourceBundle(fis1);
    	    
    	    //log.debug("value is :"+bundle.getString(PropertiesConstants.prop_selected_machine)+"_");
    	    
    	    strHostIp = bundleHost.getString( PassiveReaderPropertyConstants.HOST_IP ); //"192.168.0.200";	//Host IP
        	...
        	e.printStackTrace();
        	log.debug("Exception while loading the configurations from Passive Reader Config file. Return false.");
        	return false;
        } finally {
	    		try {
	    			if(fis != null) {
        ...
        // log.debug(hScanner +":"+ strReaderIp +":"+ m_Port +":"+ strHostIp +":"+ m_HostPort);
...
*/
		...
		if (res!=ReaderAPI._OK)
		{
            ...
            log.debug("Connect Reader Success!");
            return true;
            //æ›´æ–°å¤–è´­çŠ¶æ€�
            //UpdateStatus();
        	...
        	log.debug("Connect Reader Fail ! Rfid Reader:"+readerUsed);
        	return false;
        }
	...
	public boolean disConnectReader() {
		log.debug("disconnect reader called...");
		if(objectReaderApi.Net_DisconnectScanner() == 0) {
			return true;
		}
		else 
			return false;
	...
	class MyTimer extends Thread
	{
	    ...
	    public void run()
		{
	        ...
	        for(;;)
	        {
	            log.debug("ThreadRun!"); //åœ¨è¿™å†™ä½ è¦�è°ƒç”¨çš„æ–¹æ³•
	            try{
	            int res = -1;
	            int be_antenna	=	0;
	...
	/*                    switch(ConnectMode)
	                    ...
	                    if(result!=null) {
		                    for(EpcEntry s : result) {
		                    	//log.debug("epc:"+s.getStrEPC()+"..id:"+s.getStrID()+"..len:"+s.getStrLLen()+"..suc:"+s.getStrSuccess()+"..tim:"+s.getStrTimes());
		                    	epc_Number_Selected = s.getStrEPC();
		                    	if(null != epc_Number_Selected && epc_Number_Selected.length()>1) {
				                    ...
				                    if(PassiveRead.is_epc_Number_Selected) {
				                    	log.debug("Epc number is selected by other reader. So stoping :"+readerUsed);
				                    	setEnd(true);
				                    	break;
				                    }
				                    
			                    	//log.debug("PassiveRead.epc_Number_Selected:"+PassiveRead.epc_Number_Selected );
			                    	is_epc_Number_Selected = true;
			                    	//log.debug("Passive Reader used is :"+readerUsed);
			                    	readerUsed_epc_Number_Selected = readerUsed;
			                    	
			                    	setEnd(true);
			                    	break;
			                    	// Got one valid epc number.
		                    	}
		                    }
	                    }
	                    /*for(i=0;i<k;i++) {
...
class MyTimerAuto extends Thread
{
    ...
    public void run()
    {
                ...
                if (eventsCount > 0) {  
                    ...
                    while (iterator.hasNext()) {  
                        ...
                        if (sk.isReadable()) {
                            ...
                            log.debug("Receive message:"+ charBuffer.toString());  
        ...
        try{
            log.debug("ThreadRun Start!"); //åœ¨è¿™å†™ä½ è¦�è°ƒç”¨çš„æ–¹æ³•
