...
public class MainClass {
	...
	public static boolean storeTearWeight =  false; 
	...
	public static void main(String[] args) {
		// call Ask class & take the input from user that on which path to be going on.
		Ask.call();
		
		// when proceed pressed. Take value and callService().
		while( !Ask.proceedPressed ) {
    		System.out.println("ss:"+Ask.proceedPressed);
    		try {
				Thread.sleep(1000);
    	...
    	Ask.frame.setVisible(false);
    	System.out.println("Pressed. Value is:"+Ask.value);
    	
    	if(Ask.value.equalsIgnoreCase("Tear Weight")) {
    		storeTearWeight = true;
    	}
    	
    	// call the important service !!
		callService();
	}
	...
	public static void callService() {
		...
		ProductionStaticMont.weight_freezed = false;
		//  Just start the JFrame with dummy values.
		ProductionStaticMont.putValuesRepaint(not_Started, not_Started, not_Started, whichEpc, epcNum, weight);
		if(loadOnceEpcService() > 0) {
			log.debug("Both epc's thread dispatched. Not sure that any reader(s) is/are connected.");
			ProductionStaticMont.refreshStatusOnly(epcLoaded);
			
			if(getEpcService()) {
				log.debug("Got one epc number: Epc:"+epcNum);
				log.debug("Start validating Epc Number:"+epcNum);
				
				ProductionStaticMont.refreshStatusOnly(validatingEpcLoaded);
				...
				// validate epc number.
				epcinv = Utils.isEpcNumberValid(epcNum, storeTearWeight);
				...
				if(!epcValidFromDb) {
					...
					ProductionStaticMont.refreshStatusOnly(EPC_NOT_VALID);
					...
					ProductionStaticMont.refreshStatusOnly(EPC_VALID);
	...
	/*------------------------------------------------------------------------------- */
					...
					ProductionStaticMont.refreshStatusOnly( LET_TRUCK_IN );
					
					// Make a write request here to write PLC.
					boolean successfullyWrite = false;
					
					// value 1 , as there is on
					do {
						successfullyWrite = Utils.invokeWriteRequest(WRITE_PARAM_TRIGGER_PLC_LET_TRUCK_IN, VALUE_1);
			...
			// Now read the button value.
					...
					if(buttonValue != null && buttonValue.equals(VALUE_1)) {
						...
						ProductionStaticMont.refreshStatusOnly( GREEN_BUTTON_PRESSED );
						
 //-------------------------------------- Take weight ----------------------------------------------
						log.debug("Load weight service.");		
						if ( loadOnceWeightService() ) {
								ProductionStaticMont.refreshStatusOnly(WEIGH_SCALE_LOADED);
...
//--------------------------- Put validations. 1st stage --------------------------------------------------
		...
		//v3=1--------------------------- Store in DB, 2nd stage ------------------------------------------------------
							...
							try {
								int res = Utils.insertActivityRecord(epcinv, weight);
								if(res > 0) {
									reasonVariableValue = "1";
		...
		//v3!=1-----------------------------------------------------------------------------------
						...
						ProductionStaticMont.refreshStatusOnly( RED_BUTTON_PRESSED );
...
//--------------------------- Reading v4 & restart transaction. Take it as v4 = 1. It means restart the whole transaction---------
					...
					ProductionStaticMont.refreshStatusOnly("Truck is out. Got ok from plc to refresh for a new call.");
					...
					ProductionStaticMont.refreshStatusOnly(TRANSACTION_COMPLETE);
					CompleteRefreshCycle();
...
// --------------------------------------------------- END ------------------------------------------------------------------------
	...
	public static void CompleteRefreshCycle() {
		...
		ProductionStaticMont.refreshStatusOnly(refreshed);
	...
	public static boolean getEpcService() {
		...
		// set epc number.
		while(true) {
			if( thread_passiveRead_One.connected ) {
				...
				if( PassiveRead.is_epc_Number_Selected  && null != PassiveRead.epc_Number_Selected
					...
					if(PassiveRead.epc_Number_Selected != null && PassiveRead.epc_Number_Selected.length() > 0) {
						...
						if( weight_scale_STATUS )
							ProductionStaticMont.putValuesRepaint(started_stat, started_stat, started_stat, whichEpc, epcNum, weight);
						else 
							ProductionStaticMont.putValuesRepaint(started_stat, started_stat, not_Started, whichEpc, epcNum, weight);
	...
	public static void getWeightService() {
		...
		while(true) {
			if(ProductionStaticMont.weight_freezed) {
			 		...
			 		//&& weight != WeighingMachineSimpleRead.weight_Calculated)  
			{
				...
				//if ( null != weight && !weight.equals("") ) {
					...
					ProductionStaticMont.refreshStatusOnly(WEIGHT_TAKEN);
					break;
				}
